...
# Configura el registro para enviar mensajes a CloudWatch Logs
...
def SalidaSV_T_NS(SP_NS, SP_EOE, SV_NS, SV_EOE, Tiempo_NS, w, x, y, z, EejeSalida):
    ...
    # Salidacertera=r.centroid(EjePeatones,Conjuntorecortado)
...
def SalidaSV_T_EOE(SP_NS, SP_EOE, SV_NS, SV_EOE, Tiempo_EOE, w, x, y, z, EejeSalida):
    ...
    Salidacertera = fuzz.centroid(EejeSalida, Conjuntorecortado)
#  r.fuzz.centroid
    # Salidacertera=r.get.centroid(EjePeatones,Conjuntorecortado)
    return Salidacertera
...
def DeteccionAmbulancia(SalidaNorteSur, SalidaEsteOeste, AmbulanciaNS, AmbulanciaEOE):
    TiempoExtraNS = FuncionAmbulancia(AmbulanciaNS) if AmbulanciaNS > 0 else 0
    TiempoExtraEOE = FuncionAmbulancia(
        AmbulanciaEOE) if AmbulanciaEOE > 0 else 0

    # Calcular el tiempo total de semáforo considerando las ambulancias en cada dirección
    SalidaNS = SalidaNorteSur + TiempoExtraNS
    SalidaEOE = SalidaEsteOeste + TiempoExtraEOE

    # El tiempo de prioridad es el mayor entre las dos direcciones
    Salida = max(SalidaNS, SalidaEOE)

    # Determinar la presencia de ambulancias en cada dirección
    AmbulanciaEnNS = AmbulanciaNS > 0
    AmbulanciaEnEOE = AmbulanciaEOE > 0

    return Salida, AmbulanciaEnNS, AmbulanciaEnEOE
...
def handler(event, context):
    try:
        ...
        autos_NS = int(item.get('Promedio_V_NS'))
        peatones_NS = int(item.get('Promedio_P_NS'))
        autos_EOE = int(item.get('Promedio_V_EOE'))
        peatones_EOE = int(item.get('Promedio_P_EOE'))

        # redondeado al valor mas alto
        # NumAmbulancias = math.ceil(item.get('Promedio_Ambulancia'))
        # Convertir los valores a float y luego aplicar math.ceil()
        try:
            AmbulanciaNS = math.ceil(
                float(item.get('Promedio_Ambulancia_NS', 0)))
            AmbulanciaEOE = math.ceil(
                float(item.get('Promedio_Ambulancia_EOE', 0)))
        except ValueError:
            # Si la conversión falla, establecer el valor a 0 o manejar el error de manera apropiada
            AmbulanciaNS = 0
            AmbulanciaEOE = 0
        ...
        SalidaNS = SalidaSV_T_NS(SP_NS, SP_EOE, SV_NS, SV_EOE, Tiempo_NS,
                                 NumP_NS, NumV_NS, NumP_EOE, NumV_EOE, EejeSalida)
        SalidaEOE = SalidaSV_T_EOE(
            SP_NS, SP_EOE, SV_NS, SV_EOE, Tiempo_EOE, NumP_NS, NumV_NS, NumP_EOE, NumV_EOE, EejeSalida)

        # 2 Salidas del sistema difuso
        print("Numero de ambulancias en (N/S):"+str(AmbulanciaNS) +
              "\nNumero de ambulancias en (E/OE):" + str(AmbulanciaEOE))

        print("SalidaNS: "+str(SalidaNS))
        print("SalidaEOE: "+str(SalidaEOE))

        Salida, AmbulanciaEnNS, AmbulanciaEnEOE = DeteccionAmbulancia(
            SalidaNS, SalidaEOE, AmbulanciaNS, AmbulanciaEOE)
        ...
        # Capturar y mostrar el tipo y mensaje del error
        ...
        return {
            ...
            "mensaje_error": str(e),
            "detalle_error": error_info
        }

    return {
        "resultado": Respuesta,
        "AmbulanciaEnNS": AmbulanciaEnNS,
        "AmbulanciaEnEOE": AmbulanciaEnEOE
    }
